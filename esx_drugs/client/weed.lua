...
CreateThread(function()
	while true do
		...
		if #(coords - Config.CircleZones.WeedField.coords) < 50 then					-- 50
			SpawnWeedPlants()
		end
	end
end)
...
function ProcessWeed(xCannabis)
  ...
  TriggerServerEvent('esx_drugs:processCannabis')
	if(xCannabis <= 5) then								-- Cannabis menge die abgezogen wird!
		xCannabis = 0
	end
  local timeLeft = (Config.Delays.WeedProcessing * xCannabis) / 10000			-- Zeit die das verarbeiten benötigt!
	local playerPed = PlayerPedId()
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_drugs:canPickUp', function(canPickUp)
					if canPickUp then
						...
						Wait(10000) 									-- Zeit bis geernetet! 2000 = 2 sekunden
						ClearPedTasks(playerPed)
						Wait(1000) 									-- Wartezeit nach dem ernten! 1500 = 1,5 sekunden 
...
function SpawnWeedPlants()
	while spawnedWeeds < 10 do 										-- Anzahl Pflanzen die gleichzeitig Gespawnt werden! 10 = 1
		Wait(0)
		local weedCoords = GenerateWeedCoords()

		ESX.Game.SpawnLocalObject('bzzz_plants_weed_romance_bud_big', weedCoords, function(obj) 	-- prop_weed_02
...
function ValidateWeedCoord(plantCoord)
	if spawnedWeeds > 0 then
		...
		if #(plantCoord - Config.CircleZones.WeedField.coords) > 50 then				-- 50
			validate = false
		end
...
function GenerateWeedCoords()
	while true do
		...
		local modX = math.random(-1, 1)    								-- (-90, 90)

		Wait(100)

		math.randomseed(GetGameTimer())
		local modY = math.random(-1, 1)									-- (-90, 90)
